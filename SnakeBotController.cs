...
public class SnakeBotController : NetworkBehaviour
{
    // Settings
    ...
    private const float c_ForwardWeight = 0.5f; // INFO: weight for no rotation change
    private const float c_TurnWeight = 1.25f; // INFO: weight for left/right rotation change
    ...
    private int i_LastRotationDirection = 0; // INFO: -1 is left, 1 is right. bandaid for the jitters
    ...
    private void FixedUpdate() // INFO: keep stuff from being reliant on fps by having physics done in fixedupdate
    {
        ...
        if (l_ChangeAngle)
        {
            Vector3 targetRotation = new Vector3(0, 0, l_Angle);
    ...
    private void ServerGrowSnake()
    {
        ...
        Vector3 l_Position = i_PositionsHistory[Mathf.Clamp((i_Length - 2) * c_Gap, 0, i_PositionsHistory.Count - 2)];
        GameObject l_Part = Instantiate(i_BodyPrefab, l_Position, Quaternion.identity);
    ...
    private void DestroyLastPart()
    {
        ...
        //LeaderBoard.RequestSendServerScore(base.LocalConnection, InputNameHandler.p_Name, i_Length);
