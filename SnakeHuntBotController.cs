...
public class SnakeHuntBotController : NetworkBehaviour
{
    // Settings
    ...
    private const float c_HuntCooldown = 1f;
    ...
    private void FixedUpdate() // INFO: keep stuff from being reliant on fps by having physics done in fixedupdate
    {
        ...
        if (!i_Hunting && i_CanHunt)
        {
            ...
            int l_Collisions = Physics2D.OverlapCircleNonAlloc(l_TransPos, c_HuntRadius, l_Colliders, i_FoodLayerMask);
